...
struct FlashDriverInterface {
    ...
    flash_verify_erase:
        unsafe extern "C" fn(config: &mut FlashConfig, start: u32, length_in_bytes: u32) -> u32,
    ...
    flash_get_property:
        unsafe extern "C" fn(config: &mut FlashConfig, tag: u32, value: &mut u32) -> u32,
    reserved: [u32; 3],
    ...
    ffr_keystore_get_ac:
        unsafe extern "C" fn(config: &mut FlashConfig, activation_code: *mut u8) -> u32,
    ffr_keystore_get_kc:
        unsafe extern "C" fn(config: &mut FlashConfig, keycode: *mut u8, key_index: u32) -> u32,

    ffr_infield_page_write:
        unsafe extern "C" fn(config: &mut FlashConfig, page_data: *const u8, valid_len: u32) -> u32,
...
impl Pfr<init_state::Enabled> {
    pub fn read_cmpa(&mut self) -> Result<Cmpa, u32> {
        ...
        let ffr_get_customer_data = Self::bootloader_api_tree()
            .flash_driver
            .ffr_get_customer_data;

        Self::check_error(unsafe {
            ffr_get_customer_data(&mut self.flash_config, cmpa_bytes.as_mut_ptr(), 0, 512)
        })?;
        // heprintln!("cfpa:").ok();
    ...
    pub fn cmpa_customer_data(&mut self) -> &'static [u8] {
        ...
        unsafe { core::slice::from_raw_parts(cmpa_ptr, 224) }
    }
    ...
    pub fn read_cfpa_with_bootrom(&mut self) -> Result<Cfpa, u32> {
        ...
        let ffr_get_customer_infield_data = Self::bootloader_api_tree()
            .flash_driver
            .ffr_get_customer_infield_data;

        Self::check_error(unsafe {
            ffr_get_customer_infield_data(&mut self.flash_config, cfpa_bytes.as_mut_ptr(), 0, 512)
        })?;
        // heprintln!("cfpa:").ok();
        // dump_hex!(cfpa_bytes, 512);
    ...
    pub fn write_cfpa(&mut self, cfpa: &Cfpa) -> Result<(), u32> {
        let ffr_infield_page_write = Self::bootloader_api_tree()
            .flash_driver
            .ffr_infield_page_write;
        let cfpa_bytes: *const u8 = unsafe { core::mem::transmute(cfpa as *const Cfpa) };
        Self::check_error(unsafe {
            ffr_infield_page_write(&mut self.flash_config, cfpa_bytes, 512)
        })?;
        Ok(())
    }
    ...
    pub fn read_key_code(&mut self, key_type: KeyType) -> Result<[u8; 52], u32> {
        ...
        Self::check_error(unsafe {
            ffr_keystore_get_kc(&mut self.flash_config, bytes.as_mut_ptr(), key_type as u32)
        })?;
    ...
    pub fn read_activation_code(&mut self) -> Result<[u8; 1192], u32> {
        ...
        Self::check_error(unsafe { ffr_keystore_get_ac(&mut self.flash_config, ac.as_mut_ptr()) })?;
