use crate::{
    drivers::pins::Pin,
    peripherals::{pmc::Pmc, syscon::Syscon},
    raw,
    typestates::{
        init_state,
...
impl<State> Adc<State> {
    ...
    pub fn arm_normal_channel(&mut self, channel_id: u8) {
        self.raw
            .cmdl2
            .write(|w| unsafe { w.adch().bits(channel_id).ctype().ctype_0().mode().mode_0() });
        self.raw.cmdh2.write(|w| unsafe {
            w.avgs()
                .avgs_7()
                .cmpen()
    ...
    pub fn arm_comparator_channel(&mut self, channel_id: u8) {
        self.raw
            .cmdl1
            .write(|w| unsafe { w.adch().bits(channel_id).ctype().ctype_0().mode().mode_0() });
        self.raw.cmdh1.write(|w| unsafe {
            w.avgs()
                .avgs_7() // average 2^3 samples
                .cmpen()
    ...
    pub fn set_threshold(&mut self, low: u16, high: u16) {
        self.raw
            .cv1
            .write(|w| unsafe { w.cvl().bits(low).cvh().bits(high) })
    }
    ...
    pub fn enabled(mut self, pmc: &mut Pmc, syscon: &mut Syscon) -> Adc<init_state::Enabled> {
        ...
        self.raw.cfg.write(|w| unsafe {
            w.pwren()
                ...
                .bits(0)
                .tres()
                .clear_bit() // Do not resume interrupted captures
        });

        if self.config.conversion_delay > 0 {
            self.raw.pause.write(|w| unsafe {
                w.pausedly()
                    .bits(self.config.conversion_delay)
                    .pauseen()
                    .set_bit()
            });
        } else {
            self.raw
                .pause
                .write(|w| unsafe { w.pausedly().bits(0).pauseen().clear_bit() });
        }
...
impl<State> Adc<State> {
    ...
    pub fn read(&mut self, pin: &Pin<impl PinId, state::Analog<direction::Input>>) -> Result<u16> {
        ...
        self.raw
            .swtrig
            .write(|w| unsafe { w.bits(1 << (ChannelType::Normal as usize)) });
